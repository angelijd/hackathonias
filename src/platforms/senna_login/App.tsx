  ...
  return (
    ...
    >
      ...
      <Topbar
        ...
        userName={userName}
        isLoggedIn={activePlatform !== 'login'}
        onEditName={() => {
      ...
      <div className="relative flex-1 flex flex-col justify-between">
        ...
        {activePlatform === 'login' ? (
          <PortalLoginScreen 
            darkMode={darkMode} 
            onLoginSuccess={(name, role, contact) => {
              setUserName(name);
              setUserRole(role);
              if (contact) {
              ...
              if (role === 'aluno') {
                setActivePlatform('descoberta');
                setScreenStep('welcome');
              } else {
                setActivePlatform('professor_panel');
              }
            }}
          />
        ) : activePlatform === 'professor_panel' ? (
          <TeacherDashboard 
            darkMode={darkMode} 
            role={userRole}
            teacherContact={teacherContact}
            onLogout={() => setActivePlatform('login')} 
          />
        ) : (
          <>
              ...
              <div className="flex-1 flex flex-col justify-between">
                ...
                <main className="relative z-10 w-full max-w-[1400px] mx-auto px-5 sm:px-10 pt-8 sm:pt-14 pb-4 grid grid-cols-1 lg:grid-cols-[minmax(320px,486px)_1fr] gap-6 lg:gap-10 items-start animate-in fade-in duration-200">
                  ...
                  <section className="pt-1 flex flex-col justify-start">
                    ...
                    {/* Title / Heading */}
                    <h1 className="text-[30px] sm:text-[36px] lg:text-[39px] leading-[1.16] font-extrabold tracking-[-0.015em] max-w-[480px]">
                      <span className={darkMode ? 'text-white' : 'text-[#0B1226]'}>
                        Oi, {userName}!{' '}
                      </span>
                      <span className="text-[#FBB800] block mt-1 sm:mt-1.5">
                        Você está pronto para descobrir mais sobre você?
                      </span>
                    </h1>
                ...
                <section className="relative z-10 w-full max-w-[1400px] mx-auto px-5 sm:px-10 mt-4 sm:mt-6 pb-8 sm:pb-12 flex flex-col items-center">
                  <div className="w-full max-w-[440px] sm:max-w-[500px]">
                    ...
                    <button
                      ...
                      aria-label="Começar a minha descoberta"
                    >
                      ...
                      <span className="text-[16px] sm:text-[17px] font-extrabold text-[#0B1226] tracking-tight pr-6 select-none">
                        Começar a minha descoberta
                      </span>
            ...
            {/* SCREEN 2: ESCOLHA DO UNIVERSO DE SÉRIES */}
            {screenStep === 'preferences' && (
              <div className="animate-in fade-in duration-200">
                <Screen2Preferences
                  ...
                  onProceed={handleProceedToAssessment}
                />
              </div>
            )}

            {/* SCREEN 3: TESTE / PERGUNTAS */}
            {screenStep === 'assessment' && (
              <div className="animate-in fade-in duration-200">
                <Screen3Assessment
                  ...
                  darkMode={darkMode}
                  selectedInterests={selectedInterests}
                  interestDetail={selectedInterests[0] || 'Stranger Things'}
                  selectedExpectation={selectedExpectation}
                  testType={selectedTestType || 'critical_thinking'}
                  onBackToWelcome={handleBackToPreferences}
                />
              </div>
            )}
          </>
