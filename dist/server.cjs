...
var SUPABASE_KEY = process.env.SUPABASE_KEY || "sb_publishable_Xlmkb7AqkuTEh2MyfNSY9g_aej8kptI";
...
`;
  ...
  app.post("/api/save-fixed-test-result", (req, res) => {
    ...
    if (!platform) {
      return res.status(400).json({ error: "Par\xE2metro platform ausente." });
    }
    saveToSupabase("test_answers", {
      platform,
      test_type: null,
